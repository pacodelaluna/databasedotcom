...
module Databasedotcom
  ...
  class Client
    ...
    # Undefine the classname generated by a "describe" call to Salesforce API.
    #
    #    client.dematerialize("Contact")
    def dematerialize(classnames)
      classes = (classnames.is_a?(Array) ? classnames : [classnames]).collect do |clazz|
        if const_defined_in_module(module_namespace, clazz.to_s.to_sym)
          #module_namespace.remove_const(clazz.to_s.to_sym)
          Object.send(:remove_const, clazz.to_s.to_sym)
        end
      end
      classes.length == 1 ? classes.first : classes
    end
    ...
    def create(class_or_classname, object_attrs)
      # Hack to fix the multiple Sobject description on Salesforce side ...
      dematerialize(class_or_classname)
      class_or_classname = find_or_materialize(class_or_classname)
